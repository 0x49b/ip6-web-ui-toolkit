...
const loginFormProjector = (loginController, rootElement, loginForm) => {
  const loginButton = document.createElement('button')
  loginButton.innerHTML = 'Login'

  loginButton.onclick = () => loginForm.onLogin()

  const emailInputElement = loginTextProjector(loginForm)
  const passwordInputElement = loginPasswordProjector(loginForm)

  emailInputElement.id = 'email'
  passwordInputElement.id = 'password'

  const emailLabelElement = loginLabelProjector('Email', 'email')
  const passwordLabelElement = loginLabelProjector('Password', 'password')

  const showButtonElement = loginShowButtonProjector(loginForm)

  const notificationElement = loginNotificationProjector(loginForm)

  loginForm.onFormValidityChanged(
    valid => valid
      ? loginButton.classList.remove('disabled')
      : loginButton.classList.add('disabled')
  )

  loginForm.onVisibilityChanged( () => passwordInputElement.type = loginForm.getVisibility() ? 'text' : 'password' )
...
const loginTextProjector = loginForm => {

  const inputElement = document.createElement('input')
  inputElement.type = 'text'
  
  inputElement.oninput = () => loginForm.setEmail(inputElement.value)

  loginForm.onEmailChanged( () => inputElement.value = loginForm.getEmail() )

  loginForm.onEmailValidityChanged(
    valid => valid
...
const loginPasswordProjector = loginForm => {

  const inputElement = document.createElement('input')
  inputElement.type = 'password'

  inputElement.oninput = () => loginForm.setPassword(inputElement.value)

  loginForm.onPasswordChanged( () => inputElement.value = loginForm.getPassword() )

  return inputElement
}

const loginLabelProjector = (labelText, labelFor) => {
...
const loginShowButtonProjector = loginForm => {

  const buttonElement = document.createElement('button')
  buttonElement.innerHTML = 'show'

  buttonElement.onclick = () => {
    loginForm.setVisibility(!loginForm.getVisibility())
  }

  loginForm.onVisibilityChanged( () => buttonElement.innerHTML = loginForm.getVisibility() ? 'hide' : 'show')

  return buttonElement
}

const loginNotificationProjector = (loginForm) => {

  const pElement = document.createElement('p')

  loginForm.onLoginSucessChanged( () => pElement.innerHTML = loginForm.getLoginSuccess() ? 'Logged in successfully!' : 'Sorry we could not process your login attempt' )

  return pElement
}

export { loginFormProjector }
